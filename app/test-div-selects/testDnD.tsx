...
import React, { useState, useEffect, Children, memo,useCallback } from 'react';
...
console.log(Date.now(), 'rendering TestDnD');
    ...
    const handleSelectChange = useCallback((divId: number, slotId: number, teamId: string) => {
        console.log(Date.now(), 'handling change ', Date.now(),divId, slotId, teamId);
        ...
        // take out whatever team was previously assigned to this slot
        if (assignedTeamsMappedByCompositeId[encodeCompositeId(divId, slotId)])
            delete newAssignedTeamsMappedByTeamId[assignedTeamsMappedByCompositeId[encodeCompositeId(divId, slotId)].team.id];

        if (selectedTeam) {

            // assign dragged team to the new slot - implicitly removes it from any previous slot
            newAssignedTeamsMappedByTeamId[selectedTeam.id] = { divId, slotId, team: selectedTeam };
        } 
        ...
        console.log(Date.now(), 'handling change done');

    }, [assignedTeamsMappedByCompositeId, assignedTeamsMappedByTeamId, teams]);

    const teamSelection =             <SelectContent>
                <SelectItem key="_" value='_'>-- None --</SelectItem>
                {teams.map((team) => (
                    <SelectItem key={team.id} value={team.id}>{team.name}</SelectItem>
                ))}
            </SelectContent>

    console.log(Date.now(), 'rendering TestDnD about to return');
    return (
                        ...
                        <CardContent>
                            {divSlotIds.map((slot) =>
                                ...
                                <MTselectTeam
                                    ...
                                    divId={d.id}
                                    slotId={slot}
                                    value={assignedTeamsMappedByCompositeId[encodeCompositeId(d.id, slot)]?.team?.id || ''}
...
function TselectTeam(props: { value: string, teams: Team[], divId: number, slotId: number, onChange: (divId: number, slotId: number, teamId: string) => void, teamSelection: React.ReactNode }) {
    const handleSelectChange = (value: string) => props.onChange(props.divId, props.slotId, value);
console.log('TselectTeam render', Date.now(), props.divId, props.slotId, props.value);
    return (
        <Select value={props.value || ''} onValueChange={handleSelectChange}>
...
const MTselectTeam = memo(TselectTeam);
